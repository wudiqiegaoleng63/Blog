...
- `make verify-integration` 启动临时 MySQL 8.4，重复执行 migration 并验证数据库保持 latest/clean；
- 两个独立 Consumer 对同一批任务并发领取，真实验证 MySQL `FOR UPDATE SKIP LOCKED` 不重复执行，CI 自动运行；
- HTTP 闭环覆盖注册、登录、refresh rotation/replay、封禁/降权、文章可见性、评论入队与 Worker moderation；
- 后续集成环境补充 Redis 7.4、Milvus 和完整 API/Worker 进程级闭环；
