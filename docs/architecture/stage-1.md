...
> 本文记录 Stage 1 的认证、博客领域和任务队列契约。Stage 1 已完成；剩余真实 Compose 集成验收纳入 `stage-5.md`。
...
2. Worker 使用 MySQL 8 `FOR UPDATE SKIP LOCKED` 和主键 FIFO 扫描领取批次；领取事务使用 `READ COMMITTED`，避免 InnoDB secondary-index range lock 隐藏尚未领取的行。任务 ID 是创建顺序的稳定代理，priority 字段仍保留但当前并发安全领取不承诺严格优先级。
